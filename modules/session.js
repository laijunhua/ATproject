...
Session.prototype._create = function(res, req, next) {
    ...
    if (res.statusCode)
        res.cookie(self.options.cookie, json, {httpOnly: Boolean});
...
module.exports.install = function(framework, options) {
    ...
    session.options = { cookie: CONFIG('session-cookie'), secret: CONFIG('session-secret'), timeout: CONFIG('session-timeout')};
